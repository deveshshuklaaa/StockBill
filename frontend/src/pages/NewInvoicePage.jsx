...
export default function NewInvoicePage() {
  ...
  const totals = useMemo(() => lines.reduce((result, line) => {
    ...
    return result
  }, { subtotal: 0, tax: 0, total: 0, taxBySlab: {} }), [lines])
  ...
  function addProduct(product) {
    ...
    if (existing) {
      ...
      setLines([...lines, { key: product.id, product: product.id, productData: product, quantity: 1, rate_charged: product.default_price, tax_rate: product.tax_slab }])
    }
    setProductSearch('')
  }
  ...
  async function submit(event) {
    ...
    try {
      const payload = {
        ...
        line_items: lines.map((line) => ({ product: line.product, quantity: Number(line.quantity), rate_charged: Number(line.rate_charged), tax_rate: line.tax_rate })),
      }
  ...
  return <section className="page-section invoice-page">
    <header className="page-header invoice-header"><div><p className="eyebrow">Billing / new transaction</p><h1>New invoice</h1><p className="page-subtitle">Fast entry for the counter. Stock and tax are checked again by the server.</p></div><Link className="quiet-button" to="/products">Back to products</Link></header>
    ...
    <form onSubmit={submit} className="invoice-layout">
      <div className="invoice-workspace">
        ...
        <section className="invoice-card"><div className="section-heading"><div><div className="section-kicker">02 / Payment</div><h2>How is this sale made?</h2></div><div className="payment-toggle"><button type="button" className={paymentType === 'cash' ? 'selected' : ''} onClick={() => setPaymentType('cash')}>Cash</button><button type="button" className={paymentType === 'credit' ? 'selected' : ''} onClick={() => setPaymentType('credit')} disabled={!customer.id}>Credit</button></div></div>{!customer.id && <p className="inline-note">Walk-in sales are cash only.</p>}</section>
        <section className="invoice-card lines-card"><div className="section-heading"><div><div className="section-kicker">03 / Items</div><h2>What is going out?</h2></div><span className="line-count">{lines.length} line{lines.length === 1 ? '' : 's'}</span></div><div className="product-search"><input value={productSearch} onChange={(event) => setProductSearch(event.target.value)} placeholder="Search products by name to add..." aria-label="Search products" />{productSearch && <div className="suggestion-list product-suggestions">{filteredProducts.map((product) => <button type="button" key={product.id} onClick={() => addProduct(product)}><strong>{product.name}</strong><span>{product.current_stock} {product.unit_type} available · {money(product.default_price)}</span></button>)}{!filteredProducts.length && <div className="suggestion-empty">No matching product</div>}</div>}</div>{!lines.length ? <div className="lines-empty">Start typing above to add the first product.</div> : <div className="invoice-lines">{lines.map((line, index) => { const calculated = calculateLine(line); const overStock = Number(line.quantity) > Number(line.productData.current_stock); const fixedUnit = Number(line.productData.unit_conversion_factor) <= 1; return <div className="invoice-line" key={line.key}><div className="line-number">{String(index + 1).padStart(2, '0')}</div><div className="line-product"><strong>{line.productData.name}</strong><span>{line.productData.unit_type} · {line.productData.current_stock} available</span></div><label>Qty<input type="number" min="0.001" step={fixedUnit ? '1' : '0.001'} value={line.quantity} onChange={(event) => updateLine(line.key, 'quantity', event.target.value)} className={overStock ? 'input-warning' : ''} /></label><label>Rate<input type="number" min="0" step="0.01" value={line.rate_charged} onChange={(event) => updateLine(line.key, 'rate_charged', event.target.value)} /></label><div className="line-tax"><span>GST</span><strong>{line.tax_rate}%</strong></div><div className="line-total"><span>Total</span><strong>{money(calculated.total)}</strong></div><button type="button" className="remove-line" onClick={() => removeLine(line.key)} aria-label={`Remove ${line.productData.name}`}>×</button>{overStock && <div className="stock-warning">Quantity exceeds available stock ({line.productData.current_stock}). The server will reject this invoice.</div>}</div> })}</div>}</section>
      </div>
      <aside className="invoice-summary"><div className="summary-label">Invoice summary</div><div className="summary-customer">{customer.id ? customer.name : 'Walk-in sale'}<span>{paymentType === 'cash' ? 'Cash sale' : 'Credit sale'}</span></div><div className="summary-rows"><div><span>Subtotal</span><strong>{money(totals.subtotal)}</strong></div>{Object.entries(totals.taxBySlab).sort(([a], [b]) => Number(a) - Number(b)).map(([slab, value]) => <div key={slab}><span>{slab}% GST</span><strong>{money(value)}</strong></div>)}<div className="summary-tax"><span>Total tax</span><strong>{money(totals.tax)}</strong></div></div><div className="grand-total"><span>Grand total</span><strong>{money(totals.total)}</strong></div><button type="button" className="primary-button submit-invoice" onClick={submit} disabled={submitting || !lines.length}>{submitting ? 'Saving invoice...' : 'Create invoice'}</button><p className="summary-note">This action deducts stock immediately. Double submission is disabled while saving.</p></aside>
    </form>
  </section>
}
