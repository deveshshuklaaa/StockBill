...
export default function ProductsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter((product) => {
      if (needle && !`${product.name} ${product.sku || ''} ${product.brand || ''}`.toLowerCase().includes(needle)) return false
      if (categoryFilter && String(product.catalogue_category) !== String(categoryFilter)) return false
  ...
  async function reactivate(product) {
    try { await api.patch(`/products/${product.id}/`, { is_active: true }); await loadProducts() }
    catch (err) { setError(apiErrorMessage(err)) }
  }
    ...
    <div className="table-frame">
      <div className="table-meta">
        ...
        <div className="filter-row">
          <label className="filter-field">Search<input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Name, SKU, brand" aria-label="Search products" /></label>
          <label className="filter-field">Category<select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} aria-label="Filter by category"><option value="">All</option>{categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
          <label className="filter-field">Status<select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} aria-label="Filter by status"><option value="active">Active</option><option value="inactive">Archived</option><option value="">All</option></select></label>
        </div>
      </div>
      {busy ? <div className="empty-state">Loading products...</div> : filtered.length === 0 ? <div className="empty-state">No products match.</div> : <div className="table-scroll"><table>
        ...
        <tbody>{filtered.map((product) => { const low = Number(product.current_stock) <= Number(product.low_stock_threshold); return <tr key={product.id} className={product.is_active ? '' : 'archived-row'}>
